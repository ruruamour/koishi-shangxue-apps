...
### QQ 沙箱环境原生 Markdown 支持

QQ 官方已更新支持在沙箱环境中直接使用原生 Markdown 格式！

* **无需模板 ID：** 在沙箱环境下，你可以直接发送原生 Markdown 内容，无需预先在 QQ 开放平台申请和配置 Markdown 模板 ID。
* **更灵活的内容：** 原生 Markdown 支持更丰富的格式和动态内容，让你的机器人消息更加生动和个性化。
* **快速测试：** 在开发和测试阶段，可以直接使用 `raw` 类型配置，编辑 `.md` 文件即可实时预览效果，大大提升开发效率。

**使用建议：**

* 开发测试阶段推荐使用 `raw` 原生 Markdown 类型，可以快速迭代和调试。

* 生产环境如需使用模板 Markdown，可切换到 `markdown` 类型并配置相应的模板 ID。
